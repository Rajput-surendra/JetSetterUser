...
class SellerProfile extends StatefulWidget {
  final String? sellerID;
   String? s_id,
     ...
     bool ? isSlider,isCatBy,isSeller;

  SellerProfile({
    Key? key,
    this.sellerID,
    this.isSellerRatting,
    this.isSellerDis,
    this.isSeller,
    this.sellerName,
    this.sellerImage,
...
String? sellerName,sellerImage,sellerRatting,sellerDis,seller_address1;
  getSingleSellerApi() async {
    var headers = {
      'Cookie': 'ci_session=7a31f7b6ff260f2272900395043cd25f88cedeab'
    };
    var request = http.MultipartRequest('POST', Uri.parse('${baseUrl}get_seller_data'));
    request.fields.addAll({
      SELLER_ID:widget.sliderId.toString()
    });
    print('____Som____zzzzz__${request.fields}_________');
    ...
    if (response.statusCode == 200) {
     ...
     setState(() {
       sellerName = finalResult['data']['store_name'].toString();
       sellerImage = finalResult['data']['logo'];
       sellerRatting = finalResult['data']['rating'];
       sellerDis = finalResult['data']['store_description'];
       seller_address1 = finalResult['data']['address_map'];
       print('____Som___ssss___${seller_address1}_________');
     });
    }
    else {
  ...
  void initState() {
    getSingleSellerApi();
    print('----------seller--_${widget.storeDesc}__-----');
  ...
  Widget build(BuildContext context) {
    print('____Som______${widget.sellerName}_________');
    return Scaffold(
      key: _scaffoldKey,
                ...
                SellerDetailProviderStatus.isSuccsess
                  ...
                  return Column(
                    children: [
                      ...
                      //           Column(
                      //             children: [
                      //               Text("${sellerName.toString()}"),
                      //               Text("${sellerRatting.toString()}"),
                      //               Text("${sellerDis.toString()}"),
                      //             ],
                      //           )
                      //         ],
                      //       )
                     ...
                     // widget.isSlider== true?  GetSellerProfile(
                     //    //sellerImage: widget.sellerImage.toString(),
                     //    sellerStoreName: widget.sellerName.toString(),
                     //    sellerRating: widget.isSellerRatting,
                     //    storeDesc: widget.isSellerDis == null ? "No Description":widget.isSellerDis.toString(),
                     //    // totalProductsOfSeller: widget.totalProductsOfSeller.toString(),
                     //  ):
                      widget.isSlider == true ? sellerName == null ? Center(child: CircularProgressIndicator()):  GetSellerProfile(
                       // sellerImage: widget.sellerImage.toString(),
                        sellerStoreName:sellerName.toString(),
                        sellerRating: sellerRatting.toString(),
                        sellerAddress:seller_address1 == null? "": seller_address1.toString(),
                        storeDesc:sellerDis == null ? "No Description": sellerDis.toString(),
                        // totalProductsOfSeller: widget.totalProductsOfSeller.toString(),
                      ):
                      ...
                      GetSellerProfile(
                        ...
                        sellerStoreName: widget.sellerName.toString(),
                          ...
                          sellerAddress:widget.sellerAddress ==null? "": widget.sellerAddress,
                          storeDesc: widget.storeDesc.toString(),
                        // totalProductsOfSeller: widget.totalProductsOfSeller.toString(),
                      ),
                      Expanded(
                        child: Stack(
