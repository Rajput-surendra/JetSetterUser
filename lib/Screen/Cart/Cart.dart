...
class StateCart extends State<Cart> with TickerProviderStateMixin {
    ...
    if (isNetworkAvail) {
      try {
        ...
        apiBaseHelper.postAPICall(getCartApi, parameter).then((getdata) async {
          print('____Som______${getCartApi}_________');
    ...
    if (isNetworkAvail) {
      try {
        ...
        apiBaseHelper.postAPICall(getCartApi, parameter).then((getdata) {
          print('____Som______${parameter}_____${getCartApi}____');
          bool error = getdata['error'];
          String? msg = getdata['message'];
          if (!error) {
            var data = getdata['data'];
                  ...
                  children: <Widget>[
                    ...
                    InkWell(
                      ...
                      //
                        ...
                        //     if (isAvailable) {
                        //       print(context.read<CartProvider>().addressList.toString()+"ADRESS LHTG");
                        //       if (context.read<CartProvider>().addressList.isNotEmpty) {
  ...
  checkout() async {
    print('____Som______sssssssss______');
    // await getDeliveryChargeApi();
    // _getCart("");
  ...
  getPhonpayURL({int? i}) async {
    ...
    request.fields.addAll({
      ...
      'amount': finalTotalAmount == 0.0
          ? "${context.read<CartProvider>().oriPrice + double.parse(dChange.toString()) - double.parse("${context.read<CartProvider>().usedBalance}") - context.read<CartProvider>().promoAmt}"
          : '${finalTotalAmount + double.parse(dChange.toString()) - double.parse("${context.read<CartProvider>().usedBalance}") - context.read<CartProvider>().promoAmt} ',
    });
  ...
  Future<void> placeOrder(String? tranId) async {
    ...
    if (isNetworkAvail) {
      ...
      request.headers.addAll(headers);
      print('___ssss_______${placeOrderApi}_________');
  ...
  void confirmDialog(
      double? allAmt, finalAmt, int? selectAmt, double? onlyAmt) {
    showGeneralDialog(
  ...
  sellerStatusApi() async {
    ...
    var request = http.MultipartRequest(
        'POST', Uri.parse('$baseUrl1/check_seller_online'));
    request.fields.addAll({'seller_id': sellerId.toString()});
  ...
  Future<void> getDeliveryChargeApi() async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    sellerId = prefs.getString('sellerid');
    var headers = {
    ...
    try{
      ...
      print('____Som___address_id__seller_id_${request.fields}_________');
      request.headers.addAll(headers);
      http.StreamedResponse response = await request.send();
      if (response.statusCode == 200) {
        var result = await response.stream.bytesToString();
        var finalResult = jsonDecode(result);
        setState(() {
          dChange = finalResult['data'];
          print('____Som___address_id___${dChange}_________');
        });
      } else {
        print(response.reasonPhrase);
      }
        ...
        apiBaseHelper.postAPICall(checkCartDelApi, parameter).then(
          (getdata) async {
            ...
            if (error) {
              ...
              if (context.read<CartProvider>().payMethod == 'Wallet') {
                print("heloo");
                confirmDialog(
